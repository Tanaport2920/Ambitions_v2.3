...
uint8_t Imu::SpiReadBytes(uint8_t reg) {
    uint8_t tx_data[2] = {(uint8_t)(reg | 0x80), 0x00};  // MSB=1 for read
    ...
    return rx_data[1]; // 2バイト目にデータ
}

void Imu::SpiWriteBytes(uint8_t reg, uint8_t data) {
    uint8_t tx_data[2] = {(uint8_t)(reg & 0x7F), data};  // MSB=0 for write
    uint8_t rx_data[2] = {0, 0};
...
void Imu::Lsm6dsrPowerOn() {
    ...
    uint8_t who_am_i = SpiReadBytes(0x0F);  // WHO_AM_I
    printf("WHO_AM_I: 0x%02X\n", who_am_i);

    if(who_am_i == 0x6B) { // LSM6DSR expected = 0x6B
        HAL_Delay(10);
        // CTRL1_XL=0x10 → 1.66kHz, ±16g
        SpiWriteBytes(0x10, 0x84);
        HAL_Delay(10);

        // CTRL2_G=0x11 → 1.66kHz, 4000dps
